...
class FirefoxMainTabControl : TabControl
{
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        for (int i = 0; i <= TabPages.Count - 1; i++)
        {
            ...
            if (i == 0)
            {
                G.DrawImage(Resources.tab1, new Rectangle(TabRect.X + 19, TabRect.Y + ((TabRect.Height / 2) - 10), 22, 22));
            }
            else if (i == 1)
            {
                G.DrawImage(Resources.tab2, new Rectangle(TabRect.X + 19, TabRect.Y + ((TabRect.Height / 2) - 10), 22, 22));
            }
        }
